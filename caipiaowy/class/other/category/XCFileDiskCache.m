...
#import "XCFileDiskCache.h"
#import <UIKit/UIKit.h>

#define XCFileDiskCacheError(error) if (error) { NSLog(@"%@ (%d) ERROR: %@", \
...
NSString * const XCFileDiskCachePrefix = @"com.mumu.DiskCache";
NSString * const XCFileDiskCacheSharedName = @"default_name";

@interface XCFileDiskCache ()
...
@implementation XCFileDiskCache
...
- (id)initWithName:(NSString *)name
{
    ...
    if (self = [super init]) {
        _name = name;
        _queue = [XCFileDiskCache sharedQueue];
...
//        _didRemoveAllObjectsBlock = nil;
        ...
        NSString *pathComponent = [[NSString alloc] initWithFormat:@"%@.%@", XCFileDiskCachePrefix, _name];
        _cacheURL = [NSURL fileURLWithPathComponents:@[[paths objectAtIndex:0], pathComponent]];
        
        __weak XCFileDiskCache *weakSelf = self;
        
        dispatch_async(_queue, ^{
            XCFileDiskCache *strongSelf = weakSelf;
...
+ (instancetype)sharedCache
{
    ...
    dispatch_once(&predicate, ^{
        cache = [[self alloc] initWithName:XCFileDiskCacheSharedName];
    });
...
+ (dispatch_queue_t)sharedQueue
{
    ...
    dispatch_once(&predicate, ^{
        queue = dispatch_queue_create([XCFileDiskCachePrefix UTF8String], DISPATCH_QUEUE_SERIAL);
    });
...
- (void)trimToSize:(NSUInteger)trimByteCount block:(XCFileDiskCacheBlock)block
{
    if (trimByteCount == 0) {
...
- (void)trimToDate:(NSDate *)trimDate block:(XCFileDiskCacheBlock)block
{
    if (!trimDate)
        return;
...
- (void)trimToSizeByDate:(NSUInteger)trimByteCount block:(XCFileDiskCacheBlock)block
{
    if (trimByteCount == 0) {
        [self removeAll:block];
        return;
...
- (void)removeFileForKey:(NSString *)key block:(XCFileDiskCacheObjectBlock)block
{
    if (!key) {
        return;
    }
...
//        _willRemoveObjectBlock(self, key, nil, fileURL);
    ...
    BOOL remove = [XCFileDiskCache removeItemFromDiskAtURL:fileURL];
    if (!remove)
        return NO;
...
- (void)addFileForKey:(NSString *)key block:(XCFileDiskCacheObjectBlock)block
{
...
- (void)fileURLForKey:(NSString *)key block:(XCFileDiskCacheObjectBlock)block
{
    NSDate *now = [[NSDate alloc] init];
...
- (void)renameFileWithOldKey:(NSString *)oldKey newKey:(NSString *)newKey block:(XCFileDiskCacheObjectBlock)block
{
    if (!oldKey || !newKey)
        return;
...
- (void)updateForKey:(NSString*)key block:(XCFileDiskCacheObjectBlock)block
{
    __weak __typeof(self)weakSelf = self;
...
//            strongSelf->_willRemoveAllObjectsBlock(strongSelf);
        ...
        [XCFileDiskCache removeItemFromDiskAtURL:strongSelf->_cacheURL];
...
- (NSURL *)fileURLForKey:(NSString *)key
{
    ...
    [self fileURLForKey:key block:^(XCFileDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
...
- (void)addFileForKey:(NSString*)key
{
    ...
    [self addFileForKey:key block:^(XCFileDiskCache *cache, NSString *key, id<NSCoding> object, NSURL *fileURL) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)removeFileForKey:(NSString *)key
{
    ...
    [self removeFileForKey:key block:^(XCFileDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)renameFileWithOldKey:(NSString *)oldKey newKey:(NSString *)newKey
{
    ...
    [self renameFileWithOldKey:oldKey newKey:newKey block:^(XCFileDiskCache *cache, NSString *key, id<NSCoding> object, NSURL *fileURL) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)updateForKey:(NSString*)key
{
    ...
    [self updateForKey:key block:^(XCFileDiskCache *cache, NSString *key, id<NSCoding> object, NSURL *fileURL) {
        dispatch_semaphore_signal(semaphore);
    }];
